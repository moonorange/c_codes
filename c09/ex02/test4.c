...
void	split_strs(char *str, char *charset, char **strs_array)
{
	int	behind;
	int	forward;
	int	idx;
	...
	while (str[forward] != '\0')
	{
		if (is_in_charset(str[forward], charset) && forward == behind)
			behind++;
		else if (is_in_charset(str[forward], charset) && forward != behind)
		{
...
int		count_sep_num(char *str, char *charset)
{
	int		idx;
	int		cons;
	int		sep_num;
	...
	while (str[idx])
	{
		if (is_in_charset(str[idx], charset))
		{
			if (cons != 1)
				sep_num++;
			cons = 1;
...
char	**ft_split(char *str, char *charset)
{
	...
	if (!str)
		return (0);
	sep_num = 0;
	sep_num = count_sep_num(str, charset);
	strs_array = (char **)malloc(sizeof(char *) * (sep_num + 2));
	if (!strs_array)
		return (0);
	split_strs(str, charset, strs_array);
	return (strs_array);
}
